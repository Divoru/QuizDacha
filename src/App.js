...
function Quiz({ onFinish, sessionId, tracking }) {
  const questions = [
    {
      type: "single",
...
function LoadingScreen() {
  return (
    <div className="h-screen bg-black text-white flex items-center justify-center px-5 text-center">
      <div>
        <h1 className="mb-6 text-xl">Настраиваем цифровой профиль вашей дачи...</h1>

        <div className="h-2 bg-white/10 rounded-full overflow-hidden mb-4">
          <div className="h-full bg-green-400 animate-pulse w-full"></div>
        </div>

        <p className="text-sm text-white/50">Анализируем ответы и собираем персональный профиль</p>
      </div>
    </div>
  );
}
...
function calculateProfile(answers, idealDacha) {
  ...
  else if (mainMeaning === "Ухоженный участок, которым приятно любоваться") type = "beauty";
  else if (mainMeaning === "Чтобы всё было под контролем и ни о чем не беспокоиться")
    type = "control";

  let emotionalEntry = "neutral";
  if (firstThought === "Наконец-то отдых, хочу расслабиться") emotionalEntry = "rest";
  else if (firstThought === "Надо проверить участок и всё привести в порядок")
    emotionalEntry = "control";
  else if (firstThought === "Опять куча дел, вряд ли получится нормально отдохнуть")
  ...
  if (usage === "Живу здесь постоянно" || usage === "Бываю почти каждые выходные")
    usageType = "high";
  ...
  if (afterWeekend === "Чувствую себя скорее уставшим, чем отдохнувшим")
    fatigue = "high";
  else if (afterWeekend === "Немножко устал, но доволен") fatigue = "medium";
  else if (afterWeekend === "Отдохнул и набрался сил") fatigue = "low";
  else fatigue = "variable";

  let behavior = "mixed";
  if (fixingBehavior === "Всегда самому интересно этим заняться") behavior = "diy";
  else if (fixingBehavior === "Звоню знакомым мастерам") behavior = "trusted";
  else if (fixingBehavior === "Ищу, кто бы мог это сделать") behavior = "search";
  else if (fixingBehavior === "Всегда откладываю, пока не станет срочно") behavior = "avoid";
...
function ResultScreen({ profile, submission, sessionId, tracking }) {
  ...
  const fatigueText = {
    ...
    variable: "Похоже, многое зависит от сезона, задач и текущего состояния участка.",
  };

  const behaviorText = {
    diy: "Вы не боитесь участвовать сами и цените ощущение личного участия.",
    trusted: "Вы привыкли решать многое через проверенных людей и доверие для вас очень важно.",
    search: "Сейчас одна из главных сложностей — каждый раз заново искать, кому можно доверить задачу.",
    avoid: "Похоже, часть вопросов хочется просто отодвинуть подальше и не тратить на них силы.",
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6">
          ...
          {profile.raw.idealDacha && (
            <div className="mb-6 rounded-2xl bg-green-400/10 border border-green-400/20 p-4">
              <div className="text-sm text-green-300 mb-2">Ваша дача мечты</div>
              <div className="text-white/90 italic">“{profile.raw.idealDacha}”</div>
            </div>
          )}
          ...
          <div className="mb-4 text-white/55 text-sm">
            Если вам интересно, как такой помощник мог бы работать именно для вашей дачи —
            можно оставить предварительный интерес. Мы покажем, как это может выглядеть на практике.
          </div>
      ...
      {submitted && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center px-5 z-50">
          <div className="bg-white text-black rounded-3xl p-6 max-w-sm w-full text-center">
            <h2 className="text-xl font-semibold mb-4">Интерес зафиксирован ✅</h2>

            <p className="text-sm mb-6">
              Спасибо. Мы увидели ваш интерес и свяжемся с вами, когда будем готовы показать,
              как такой цифровой помощник может работать именно для вашей дачи.
            </p>
